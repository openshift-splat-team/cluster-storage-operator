# cluster-storage-operator Development Guide
...
This guide covers the development workflow for cluster-storage-operator.
...
```bash
git clone https://github.com/openshift-splat-team/cluster-storage-operator.git
cd cluster-storage-operator
```
...
# Or directly
go build -o bin/cluster-storage-operator ./cmd/...
```
...
# Or with podman
podman build -t cluster-storage-operator:latest .
```
...
# Run
./bin/cluster-storage-operator --help
```
...
# Or via command line
./bin/cluster-storage-operator --log-level=debug
```
...
# Debug
dlv debug ./cmd/cluster-storage-operator
```
...
- `make check`
- `make update`
- `make generate-kustomize-manifests`
- `make clean`
...
- [Testing Guide](cluster-storage-operator_TESTING.md) - Test suites and strategies
